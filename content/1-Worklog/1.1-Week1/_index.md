---
title: "Week 1 Worklog"
date: 2024-01-01
weight: 1
chapter: false
pre: " <b> 1.1. </b> "
---

### Week 1 Objectives

* Become familiar with the First Cloud AI Journey (FCAJ) internship program and learning roadmap.
* Learn fundamental Cloud Computing concepts and the AWS ecosystem.
* Set up an AWS account for learning and hands-on practice.
* Explore AWS Identity and Access Management (IAM) and basic account security mechanisms.

### Activities Completed During This Week

| Day | Tasks | Start Date | Completion Date | References |
|------|---------|-------------|----------------|------------|
| 2 | - Reviewed internship regulations and FCAJ learning roadmap | 11/05/2026 | 11/05/2026 | |
| 3 | - Learned AWS and Cloud Computing fundamentals <br> - Studied AWS service categories: Compute, Storage, Database, and Networking <br> - Explored AWS Global Infrastructure | 12/05/2026 | 12/05/2026 | https://cloudjourney.awsstudygroup.com/ |
| 4 | - Learned AWS Free Tier and AWS Credit concepts <br> - Created an AWS Account <br> - Configured Virtual MFA | 13/05/2026 | 13/05/2026 | https://000001.awsstudygroup.com/ |
| 5 | - Studied AWS IAM <br> - Created IAM Users and IAM Groups <br> - Learned IAM Policies and permission management | 14/05/2026 | 14/05/2026 | https://000002.awsstudygroup.com/ |
| 6 | - Learned IAM Roles <br> - Practiced Switch Role <br> - Cleaned up lab resources and reviewed weekly knowledge | 15/05/2026 | 15/05/2026 | https://000002.awsstudygroup.com/ |

### Week 1 Achievements

* Gained a fundamental understanding of Cloud Computing concepts.

* Learned the major AWS service categories:
  * Compute
  * Storage
  * Database
  * Networking

* Understood AWS Global Infrastructure, including:
  * Regions
  * Availability Zones
  * Edge Locations

* Successfully created an AWS account for learning and hands-on practice.

* Enabled Virtual MFA to improve AWS account security.

* Understood the purpose of AWS Identity and Access Management (IAM).

* Successfully practiced:
  * IAM Users
  * IAM Groups
  * IAM Policies
  * IAM Roles
  * Switch Roles

* Learned basic AWS access control and account security principles.

* Completed the foundation phase for upcoming Networking and Infrastructure topics.